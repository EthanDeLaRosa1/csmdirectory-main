...
import { exportXlsxSheets, exportPdf } from "@/lib/paz/exporters";
import { availableDatasets } from "@/lib/paz/datasets";
...
export function Dashboard() {
  ...
  async function handleFileObject(file: File | null | undefined) {
    if (!file) return;
    setError(null);
    try {
      const text = await file.text();
  ...
  const numericStats = useMemo(() => {
    if (!parsed) return [] as { column: string; count: number; sum: number; avg: number; min: number; max: number }[];
    return parsed.headers
      .map((h, idx) => {
        if (!isNumericColumn(parsed.rows, idx)) return null;
        ...
        return { column: h || `col ${idx + 1}`, count, sum, avg, min, max };
      })
      .filter(Boolean) as { column: string; count: number; sum: number; avg: number; min: number; max: number }[];
  }, [parsed]);

  return (
    <div className="report">
      <form className="bg-card text-card-foreground border border-border/60 shadow-sm rounded-xl p-6">
        <h2 className="card-title inline-flex items-center gap-2"><FileSpreadsheet className="w-5 h-5 text-muted-foreground" /> Reports & Dashboard</h2>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div>
            <label className="text-sm font-medium text-foreground block mb-1">Data source</label>
            <select value={sourceId} onChange={(e) => pickSource(e.target.value)} className="rounded-lg border border-input bg-background/50 px-3 py-2 text-sm">
              <option value="">Select live dataset…</option>
              {liveSets.map((d) => <option key={d.id} value={d.id}>{d.label}</option>)}
            </select>
          </div>
          {sourceId && (
            <button type="button" className="inline-flex items-center justify-center rounded-lg bg-muted/10 px-3 py-1 text-sm" onClick={() => loadLive(sourceId)}>↻ Refresh</button>
          )}
          {parsed && <button type="button" className="inline-flex items-center justify-center rounded-lg border border-input bg-background px-3 py-1 text-sm" onClick={clear}>Clear</button>}
        </div>
        {liveSets.length > 0 && (
        ...
        {sourceId === "" && (
          <label
            className="file-drop mt-3 w-full border-2 border-dashed border-border/60 rounded-lg p-4 flex items-center gap-4 bg-muted/5 hover:bg-muted/10 cursor-pointer"
            onDragOver={(e) => e.preventDefault()}
            onDrop={async (e) => { e.preventDefault(); const f = e.dataTransfer.files?.[0]; await handleFileObject(f); }}
            style={{ marginTop: 10 }}
          >
            <input className="sr-only" type="file" accept=",.csv,text/csv" onChange={(e) => onFile(e)} />
            <FileSpreadsheet className="w-6 h-6 text-muted-foreground" />
            <div>
              <div className="font-medium">Upload CSV</div>
              <div className="text-sm muted-line">Click to select or drag & drop a CSV file here</div>
            </div>
            <div style={{ flex: 1 }} />
            <div className="text-sm text-muted-foreground">{fileName && !sourceId ? `📄 ${fileName}` : "No file chosen"}</div>
          </label>
        )}
